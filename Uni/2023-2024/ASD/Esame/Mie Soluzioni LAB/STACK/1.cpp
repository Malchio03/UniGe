...
Stack stack_intersection(Stack& s1, Stack& s2) {
    Stack s3 = EMPTYSTACK;  // Stack risultato
    ...
    while (!isEmpty(s1) && !isEmpty(s2)) {
        ...
        if (top1 == top2) {  
            push(top1, s3);  // Inseriamo l'elemento comune in s3
            pop(s1);  // Rimuoviamo l'elemento da entrambi gli stack
            pop(s2);
        } else if (top1 < top2) {
            pop(s1);  // L'elemento di s1 non è in s2, quindi lo scartiamo
        } else {
            pop(s2);  // L'elemento di s2 non è in s1, quindi lo scartiamo
        }
    }
