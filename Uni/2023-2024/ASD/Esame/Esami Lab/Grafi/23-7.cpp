...
int pesoTotaleArchi(const Grafo& g) {
   ...
   for (int i = 0; i < g.numVertici; ++i) {
      for (int j = 0; j < g.numVertici; ++j) {
         if (g.matrice[i][j] != 0) {      // se in posozione i j esiste un arco allora sommo il valore
            sum += g.matrice[i][j];    
         }
      }
   }
...
bool sonoCollegatiDaUnVertice(const Grafo& g, VerticeID v1, VerticeID v2, VerticeID& vlink) {
    ...
    vlink= elementoInesistente;       // istruzione inserita solo per non avere problemi di compilazione
