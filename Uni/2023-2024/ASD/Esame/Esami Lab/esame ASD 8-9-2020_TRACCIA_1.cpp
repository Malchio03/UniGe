...
Stack stack_intersection(Stack& s1, Stack& s2) {
    Stack s3 = EMPTYSTACK; // Stack risultante
    Stack cur1 = s1;       // Puntatore temporaneo per s1
    Stack cur2;

    while (cur1 != EMPTYSTACK) {
        cur2 = s2; // Ripristino cur2 alla testa di s2
        while (cur2 != EMPTYSTACK) {
            if (cur1->elem == cur2->elem) {
                // Aggiungi l'elemento in cima a s3
                push(cur1->elem, s3);
                break; // Esci dal ciclo interno, trovato il match
            }
            cur2 = cur2->next;
        }
        cur1 = cur1->next;
    ...
    return s3; // s3 è già ordinato in modo decrescente grazie al push
}
