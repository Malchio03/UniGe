...
/****************************************************************/
bool stackk::isEmpty(const Stack& s) {
    return (s == nullptr); // ISTRUZIONE AGGIUNTA AI SOLI FINI DELLA COMPILAZIONE
}

/****************************************************************/
void stackk::push(const Elem x, Stack& s) {
    cell *aux = new cell; 
    aux->elem=x;
    aux->next=s;
    s = aux;
...
/****************************************************************/
Elem stackk::top(Stack& s) {
    if (isEmpty(s)) return EMPTYELEM;
    else return s->elem;
}

/****************************************************************/
void stackk::make_empty(Stack& s) {
    while (s != EMPTYSTACK) {
        Stack temp = s -> next;
        delete s;
        s = temp;
...
Stack stack_intersection(Stack& s1, Stack& s2) {
    ...
    // Continuare fino a che almeno uno degli stack è vuoto
    while (temp1 != EMPTYSTACK ) {
        temp2 = s2;
        while (temp2 != EMPTYSTACK) {
