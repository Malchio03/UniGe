...
// window
const char* window_title = "02a - move ball";
...
Ball::Ball ()
{
    ...
    float ball_px = ((float) window_width / 2.0) - radius;
    float ball_py = (float) window_height - paddle_initial_size.y - (radius * 2);
    pos = {ball_px, ball_py};
...
Paddle::Paddle ()
{
    ...
    float paddle_px = ((float) window_width / 2.0) - (size.x / 2.0);
    float paddle_py = (float) window_height - size.y;
