...
struct State
{
    ...
    unsigned active_shape = 0, active_color = 0, selected_shape = -1;
...
{
    // if (gs.selected_shape != -1 && textEnter.unicode == '\b')
    // {
...
{
    if (gs.selected_shape != -1 && keyPressed.code==sf::Keyboard::Key::Backspace)
    {
        gs.shapes.erase(gs.shapes.begin() + gs.selected_shape);
        gs.selected_shape = -1;
    }
...
void handle(const sf::Event::MouseMoved &mouseMoved, State &gs)
{
    ...
    {
        gs.shapes[gs.selected_shape].pos.x += mouseMoved.position.x - gs.mouse_pos.x;
        gs.shapes[gs.selected_shape].pos.y += mouseMoved.position.y - gs.mouse_pos.y;
...
void handle(const sf::Event::MouseButtonPressed &mouseBP, State &gs)
{
    ...
    else // canvas
    {
        ...
        else if (but == sf::Mouse::Button::Right) // select shape
        {
            gs.mouse_pos = pos;
            for (int i=0; i< gs.shapes.size();i++) 
                if (dist(gs.shapes[i].pos,pos)<gs.shapes[i].size)
                {
                    gs.selected_shape = i;
                    break;
                }
...
void doGraphics(State &gs)
{
    ...
    for (int i = 0; i < gs.shapes.size(); i++)
    {
        ...
        {
            toDraw.setOutlineColor(LIGHT_GRAY);
            toDraw.setOutlineThickness(-3.f);
        }
        else
            toDraw.setOutlineThickness(0.f);
        if (s.type > 0)
