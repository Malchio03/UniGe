...
/*** TO BE DONE START ***/
clock_gettime(CLOCK_TYPE, &send_time);	// usiamo clock type dichiarato sul file h, se cambiamo il tipo di orologio almnejo non dobbiamo cambiare qusya riga
...
if (gai_rv != 0)
	fail_errno("Error on getaddrinfo()");	

/*** TO BE DONE END ***/

    /*** Print address of the Pong sersver before trying to connect ***/
...
/*** TO BE DONE START ***/
if (write(tcp_socket, request, strlen(request)) == -1) 	// strlen sarebbe la lunghezza della stringa, ci serve perchè write non sa automaticamente quanto deve leggere dal buffer.
