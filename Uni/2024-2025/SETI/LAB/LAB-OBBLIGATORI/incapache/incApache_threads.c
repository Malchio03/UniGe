...
/*** TO BE DONE 8.1 START ***/
	...
	if(pthread_mutex_lock(&threads_mutex) != 0) 
		fail_errno("join_all_threads error: fail to LOCK"); 

	// compute the index i of the thread to join
	i = to_join[conn_no] - thread_ids;

	// call pthread_join() on thread_ids[i]
	...
	if(pthread_mutex_unlock(&threads_mutex) != 0) 
		fail_errno("join_all_threads error: fail to UNLOCK");
...
/*** TO BE DONE 8.1 START ***/
	...
	if(pthread_mutex_lock(&threads_mutex) != 0) 
		fail_errno("join_prev_thread error: fail to LOCK");
	...
	if(pthread_mutex_unlock(&threads_mutex) != 0) 
		fail_errno("join_prev_thread error: fail to UNLOCK");
...
/*** TO BE DONE 8.0 START ***/
	...
	if(pthread_mutex_lock(&mime_mutex) != 0)
		fail_errno("get_mime_type error: fail to LOCK");
...
/*** TO BE DONE 8.0 START ***/
	...
	if(pthread_mutex_unlock(&mime_mutex) != 0)
		fail_errno("get_mime_type error: fail to UNLOCK");
